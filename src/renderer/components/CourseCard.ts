...
class CourseCard extends HTMLElement {
  private course: Course // Replace 'any' with your Course type/interface if available
  ...
  private async handleCourseClick() {
    const canOpen = await window.courseAPI.validateAccess(this.course.id)
    if (canOpen.success && canOpen.canAccess) {
      const result = await window.courseAPI.getCourseContent(this.course.id)
  ...
  private render() {
    this.className = "max-w-sm w-full lg:max-w-full lg:flex mb-6"
    this.innerHTML = `
            ...
            <div class="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b-lg lg:rounded-b-none lg:rounded-r-lg p-4 flex flex-col justify-between leading-normal">
                <div class="mb-8">
                    <p class="text-sm text-gray-600 flex items-center">
                        ${
                          this.course.accessCount >= this.course.maxAccess
                        ...
                        Visualizaciones: ${this.course.accessCount}/${
      this.course.maxAccess
    }
                    </p>
                ...
                <div class="flex items-center">
                    ...
                    <button class="open-course-btn bg-orange-600 text-white py-1 px-3 rounded mt-2 ml-auto">
                        Abrir curso
                    </button>
                </div>
            </div>
    ...
    const button = this.querySelector(".open-course-btn")
    if (button) {
      button.addEventListener("click", () => this.handleCourseClick())
    }
  }
}
